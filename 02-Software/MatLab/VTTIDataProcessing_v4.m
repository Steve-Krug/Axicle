...
files = dir('C:\Users\slkru\OneDrive - Axicle\Public\9. Data\VTTI Truck IMU Data\Initial Data\Separated\*.csv');
...
for i = 1:length(files)
    ...
    csvData{i} = readtable(string(fullpaths(1,i)));
    rollFlag1 = true;
    %% Find Event
    ...
    if mean(Az_g) > 0 %This checks if the Az channel is reversed. Az should read negative, otherwise accel calculated angle will be wrong and throw off complementary filter
        Az_g = -Az_g;
    ...
    if abs(mean(Wx_dps)) > 7 %sometimes the gyros are statically broken/reading high, ignore data set if this happens
        rollFlag1 = false;
    end
