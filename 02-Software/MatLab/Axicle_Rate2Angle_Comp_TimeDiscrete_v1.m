function [angleAccel_LP, angleComp, anglePureIntegration_HP, gyro_LP] = Axicle_Rate2Angle_Comp_TimeDiscrete_v1(dt, rate_dps, thetaX_LP_Accel_deg, thetaX_deg_Complementary_deg, thetaX_Integration_HP_deg, gyro_LP_dps, Ay_g, Az_g, tau_s, tau_s_hp)
...
% gyro_LP_dps = zeros(2, 1);
% gyro_HP_dps = zeros(length(rate_dps), 1);
% thetaX_Accel_deg = zeros(2, 1); %this gets rewritten during calc

% thetaX_LP_Accel_deg = zeros(length(rate_dps), 1);
% thetaX_GyroIntegration_deg = zeros(length(rate_dps), 1); %this gets rewritten during calc
...
thetaX_GyroIntegration_deg = (thetaX_deg_Complementary_deg(i-1) + gyro_HP_dps*dt); %Pure integration. thetaX_deg_comp(i-1) was the error from before (used to be thetaX_deg_fromIntegration(i))

thetaX_Integration_HP_deg(i) = (thetaX_Integration_HP_deg(i-1) + gyro_HP_dps*dt); %Pure integration. thetaX_deg_comp(i-1) was the error from before (used to be thetaX_deg_fromIntegration(i))

thetaX_deg_Complementary_deg(i) = ( alpha_comp * thetaX_LP_Accel_deg(i) ) + ( (1-alpha_comp) * thetaX_GyroIntegration_deg ); %Apply acceleration complementary filter

% end

angleAccel_LP = thetaX_LP_Accel_deg(i);
angleComp = thetaX_deg_Complementary_deg(i);
