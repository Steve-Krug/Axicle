% trailer_mass_cg_testing_Az_SBG_v1
% Notes: 
...
ts = sbg_1{1,1}.TimeStamp(3) - sbg_1{1,1}.TimeStamp(2);
fs = 1/ts;
...
for j = 1:length(n1_1)
    ...
    [freq{j}, mag{j}, phase] = fft_lite_v2(fs, sbg_1{1,1}.AccelerometerZ);
...
%low cg 1.527m
[freq_Sim1, mag_sim1, ~] = fft_lite_v2(fs_sim, data{1,1}.AV_R);
sgf_Sim1 = sgolayfilt(mag_sim1, sg_order, sg_framelength );

%high cg 2.0m
[freq_Sim2, mag_sim2, ~] = fft_lite_v2(fs_sim, data{1,2}.AV_R);
sgf_Sim2 = sgolayfilt(mag_sim2, sg_order, sg_framelength );

%empty
[freq_Sim3, mag_sim3, ~] = fft_lite_v2(fs_sim, data{1,3}.AV_R);
sgf_Sim3 = sgolayfilt(mag_sim3, sg_order, sg_framelength );

%front high
[freq_Sim4, mag_sim4, ~] = fft_lite_v2(fs_sim, data{1,4}.AV_R);
sgf_Sim4 = sgolayfilt(mag_sim4, sg_order, sg_framelength);

%front low
[freq_Sim5, mag_sim5, ~] = fft_lite_v2(fs_sim, data{1,5}.AV_R);
sgf_Sim5 = sgolayfilt(mag_sim5, sg_order, sg_framelength);


figure(2)
hold on
title('Nano1 Wx vs. TruckSim Wx: SG Filter, Runs Averaged')
plot(mean_fs1, mean_sg1, 'r')
plot(mean_fs5, mean_sg5, 'b')
plot(mean_fs3, mean_sg3, 'g')
plot(mean_fs2, mean_sg2, 'k')
