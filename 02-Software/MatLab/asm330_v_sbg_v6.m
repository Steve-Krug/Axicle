...
time_sbg_zeroed_s = (sbgData.TimeStamp - sbgData.TimeStamp(end)) +  (-3.3300);
...
for i = 2:length(Wx_dps)
    ...
    thetaX_deg_comp(i) = ((1-alpha) * thetaX_deg_HP(i)) + (alpha * thetaX_deg_fromAccel(i)); %Apply acceleration complementary filter
...
plot(time_sbg_zeroed_s, sbgData.Roll, time_zeroed_s, thetaX_deg_HP, time_zeroed_s, thetaX_deg_fromAccel, time_zeroed_s, thetaX_deg_LP, time_zeroed_s, thetaX_deg_fromIntegration, time_zeroed_s, thetaX_deg_comp)
