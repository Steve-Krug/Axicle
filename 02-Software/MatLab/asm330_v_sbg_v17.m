...
close all
clc
% C:\Users\SteveKrug\OneDrive - Axicle\Public\15. GitHub Repo\Axicle\02-Software\MatLab
% C:\Users\slkru\OneDrive - Axicle\Public\15. GitHub Repo\Axicle\02-Software\MatLab

load('C:\Users\SteveKrug\OneDrive - Axicle\Public\15. GitHub Repo\Axicle\02-Software\MatLab\AxicleExport_554pm_Jan20_Hindry_NoLeverArm_NoGPS_Processed.mat');

sbgData = readtable('C:\Users\SteveKrug\OneDrive - Axicle\Public\15. GitHub Repo\Axicle\02-Software\MatLab\SBGCenterExport_554pm_Jan20_2024_Hindry_NoLeverArm_NoGPS.txt');


time_zeroed_s = time_s - time_s(1);
time_sbg_zeroed_s = (sbgData.TimeStamp - sbgData.TimeStamp(1)) + (-1.432 + 4.792) - 0.903;
...
% tau_s = linspace(3, 8, 20);
% errorRMSE = zeros(length(tau_s),1);

% for i = 1:length(tau_s)
tau_s_hp = 10;
tau_s = 5.6315;
[thetaX_accel, thetaX_comp_deg, angleHP, angleIntegration] = Rate2Angle_Complementary_HP_v10(time_s_clipped, Wx_dps_clipped, Ay_g_clipped, Az_g_clipped, tau_s, tau_s_hp);

% errorRMSE = rmse(thetaX_comp_deg, sbgRoll_deg_clipped');
...
% tau_s_optimal = 5.6315;
% [thetaX_accel, thetaX_comp_deg] = Rate2Angle_Complementary_HP_v9(time_s_clipped, Wx_dps_clipped, Ay_g_clipped, Az_g_clipped, tau_s_optimal);
...
grid on
ylabel('Angle [deg]')
legend('SBG Angle Est. "Truth"','Comp.')
Plotter(1)

figure(4)
hold all
plot(time_s_clipped, thetaX_accel, time_s_clipped, thetaX_comp_deg, time_s_clipped, angleHP, time_s_clipped, angleIntegration)
xlim([lowerClip_s upperClip_s])
ylim([-25 25])
legend('thetaX Accel', 'ASM Complementary', 'AngleHP', 'pure integration')
Plotter(1)
% test = sbgRoll_deg_clipped';
% subplot(3,1,3)
% plot(time_s_clipped,(thetaX_comp_deg - test))
% xlim([lowerClip_s upperClip_s])
% ylim([-3 6])
% legend('SBG minus ASM Complementary')
% Plotter(1)
