function [angleAccel, angleComp, angleHP, angleIntegration] = Rate2Angle_Complementary_HP_v10(time_s, rate_dps, Ay_g, Az_g, tau_s, tau_s_hp)
% Rate2Angle_v6.m
% 1/24/24
% S. Krug
% Notes: v6 is for calculating new drift rate when ay & Wx are low
...
for i = 2:length(rate_dps)
    ...
    thetaX_deg_fromIntegration(i) = (thetaX_deg_comp(i-1) + rate_dps(i)*dt); %Pure integration of gyro data

    % thetaX_deg_LP(i) = (thetaX_deg_fromIntegration(i) * alpha_hp) + (1-alpha_hp) * thetaX_deg_LP(i-1); %Low pass the gyro data
...
angleIntegration = thetaX_deg_fromIntegration;
