...
time_sbg_zeroed_s = (sbgData.TimeStamp - sbgData.TimeStamp(end)) + (5.470-4.038) - 4.05;
...
%Complementary Alpha
tau_s = 500; % Time constant, seconds, needs to be long enough to filter out any aggressive lateral acceleration maneuvers
...
hold on
title('SBG Angle v. ASM Complementary')
plot(time_sbg_zeroed_s, sbgData.Roll, time_zeroed_s, thetaX_deg_complementary_HP, time_zeroed_s, thetaX_deg_C_LP, time_zeroed_s, thetaX_deg_fromIntegration)
legend('SBG', 'ASM Complementary, Tau = x with HP',  'Low Pass', 'pure integration')
% xlim([-215 -175])
xlabel('Time [s]')
ylabel('Roll Angle [deg]')
Plotter(1)
