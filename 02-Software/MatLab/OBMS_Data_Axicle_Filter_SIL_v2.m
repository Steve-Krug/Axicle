...
% file names
% 2/4/24 - Revision to do OBMSFOT2 file name data
...
plotEnable = false;
controlThreshold = false;
Rate_AnglePlotEnable = true;
resample_rate = 15; %hz
allRunTimeTotal_s = 0; %for adding up entire time processed
arrestedRolloverCounter = 0; %For storing the signals of interest that triggered lookup table threshold
failCounter = 0;
fs = 10; %hz
dt = 1/fs; %s
%% Load Data

files = dir('G:\Timeseries\*OBMSFOT2_*.csv');
fullpaths = string(fullfile({files.folder}, {files.name}));

for i = 1:50 %13 'FD' data sets
    %Load first 'FD' data set. Each file is about 800MB...
    index_Ax = strcmp(fullpaths,'G:\Timeseries\OBMSFOT2_Accel_X_' + string(i)+ '.csv');
    all_data_Ax = table2array(readtable(fullpaths(index_Ax)));
    fprintf('Ax Loaded...1/6  ')
    index_Ay = strcmp(fullpaths,'G:\Timeseries\OBMSFOT2_Accel_Y_' + string(i)+ '.csv');
    all_data_Ay = table2array(readtable(fullpaths(index_Ay)));
    fprintf('Ay Loaded...2/6  ')
    index_Az = strcmp(fullpaths,'G:\Timeseries\OBMSFOT2_Accel_Z_' + string(i)+ '.csv');
    all_data_Az = table2array(readtable(fullpaths(index_Az)));
    fprintf('Az Loaded...3/6  ')
    index_Wx = strcmp(fullpaths,'G:\Timeseries\OBMSFOT2_Gyro_X_' + string(i)+ '.csv');
    all_data_Wx = table2array(readtable(fullpaths(index_Wx)));
    fprintf('Wx Loaded...4/6  ')
    index_Wy = strcmp(fullpaths,'G:\Timeseries\OBMSFOT2_Gyro_Y_' + string(i)+ '.csv');
    all_data_Wy = table2array(readtable(fullpaths(index_Wy)));
    fprintf('Wy Loaded...5/6  ')
    index_Wz = strcmp(fullpaths,'G:\Timeseries\OBMSFOT2_Gyro_Z_' + string(i)+ '.csv');
    ...
    for j = 1:length(new_id_Ax) %there are 1000 individual data sets per "FD"
        ...
        %conditions
        rollFlag1 = true;
        %reset control threshold for new run
        controlThreshold = false;
        ...
        %% Calculate if signal passed over control threshold

        %calcs
        rollAngle_deg = [25 20 15 12.5 10]';
        rollRate_dps = [0 10 20 30 40]';
        controlTable =  table(rollAngle_deg, rollRate_dps);
        controlTableInterp = interp1(controlTable.rollAngle_deg, controlTable.rollRate_dps);


        % if signal > controlshteshold
        %
        ...
        if true(Rate_AnglePlotEnable)
            figure(5)
            hold on
            if max(abs(thetaX_comp_deg)) > 20 & true(rollFlag1) % Enter statement here that plots the data if it's over the lookup table threshold
            ...
            elseif true(rollflag) & true(controlThreshold) % Store run that was caught by control lookup table


                
                arrestedRolloverCounter = arrestedRolloverCounter + 1;

                AxStored{arrestedRolloverCounter} =
                AyStored{arrestedRolloverCounter} =
                AzStored{arrestedRolloverCounter} =
                WxStored{arrestedRolloverCounter} =
                WyStored{arrestedRolloverCounter} =
                WzStored{arrestedRolloverCounter} =
                TimeStored{arrestedRolloverCounter} =



                hold on
                plot(abs(thetaX_comp_deg), abs(Wx_100_dps.Data), 'r')
                plot(controlTable.rollAngle_deg, controlTable.rollRate_dps)
                Plotter(1)
                xlabel('Roll Angle [deg]')
                ylabel('Roll Rate [deg/s]')
