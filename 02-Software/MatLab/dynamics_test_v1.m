...
TRAIL_mass_kg = linspace(22142-3*2089, 22142+3*2089, runs)'; %2.1 +- 0.15*3
...
for j = 1:length(v_mph)
    ...
    input_array((j-1)*runs+1:j*runs,2) = TRAIL_mass_kg(j); %trail_cgheight
end
...
[X, Y] = meshgrid(v_mph, TRAIL_mass_kg); %z must be a runs x runs grid with results
Z = spin';
figure(3)
