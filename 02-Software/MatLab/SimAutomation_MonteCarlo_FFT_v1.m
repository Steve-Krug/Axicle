...
for i = 1:num_runs
    ...
    set_param(block1{5,1}, 'Value', 'SSTART_sim');
    set_param(block1{6,1}, 'Value', 'SSTOP_sim');
    ...
    [data{i}(:,1), data{i}(:,2) , ~] = fft_lite_v2(fs, Ax_S1);
    [~, data{i}(:,3), ~] = fft_lite_v2(fs, Ay_S1);
    [~, data{i}(:,4), ~] = fft_lite_v2(fs, Az_S1);
    [~, data{i}(:,5), ~] = fft_lite_v2(fs, AVx_S1);
    [~, data{i}(:,6), ~] = fft_lite_v2(fs, AVy_S1);
    [~, data{i}(:,7), ~] = fft_lite_v2(fs, AVz_S1);

    [~, data{i}(:,8), ~] = fft_lite_v2(fs, Ax_S2);
    [~, data{i}(:,9), ~] = fft_lite_v2(fs, Ay_S2);
    [~, data{i}(:,10), ~] = fft_lite_v2(fs, Az_S2);
    [~, data{i}(:,11), ~] = fft_lite_v2(fs, AVx_S2);
    [~, data{i}(:,12), ~] = fft_lite_v2(fs, AVy_S2);
    [~, data{i}(:,13), ~] = fft_lite_v2(fs, AVz_S2);

    [~, data{i}(:,14), ~] = fft_lite_v2(fs, Ax_S5);
    [~, data{i}(:,15), ~] = fft_lite_v2(fs, Ay_S5);
    [~, data{i}(:,16), ~] = fft_lite_v2(fs, Az_S5);
    [~, data{i}(:,17), ~] = fft_lite_v2(fs, AVx_S5);
    [~, data{i}(:,18), ~] = fft_lite_v2(fs, AVy_S5);
    [~, data{i}(:,19), ~] = fft_lite_v2(fs, AVz_S5);
