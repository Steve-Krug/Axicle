...
load('C:\Users\slkru\OneDrive - Axicle\Public\8. Test\231013_SEA_RollTesting\seaData.mat')
...
dt = mean(gradient(axicleData1(:,2)));
%% Filters
...
filt_freq = [0.05/(newFreq/2);...   %Accelerations high pass frequency
    ...
    5/(newFreq/2);...             %general low pass settings
    5/(newFreq/2);...             %Isolate MT filter, Acc
...
tau_s = 2; % Time constant, seconds, needs to be long enough to filter out any aggressive lateral acceleration maneuvers
...
for i = 1:length(Wx_dps_filt)
    % thetaX_deg_fromAccel_test = atan(AySimNoise_g(i)/AzSimNoise_g(i))*57.3;

    thetaX_deg_fromAccel_test = -unwrap(atan2(Ay_g_filt(i), Az_g_filt(i) + pi) * 180/pi);
    ...
    thetaX_deg_loop = (1-alpha) * (thetaX_deg_loop + Wx_dps_filt(i)*dt) + (alpha) * thetaX_deg_fromAccel(i);
    thetaX_deg_complementary(i) = thetaX_deg_loop;

    thetaX_deg_loop2 = (thetaX_deg_loop2 + Wx_dps_filt(i)*dt);
...
figure(9)
subplot(2,1,1)
plot(time_s, thetaX_deg_fromIntegration, time_s, thetaX_deg_complementary)
xlabel('Time [s]')
ylabel('Roll [deg]')
% ylim([-20 20])
% xlim([62620 62620+100])
legend('Pure Integration','Complementary Filtered Roll')
...
subplot(2,1,2)
plot(time_s, Ay_g, time_s, Ay_g_filt)
...
figure(13)
hold on
plot(time_s_sea_zeroed, Tx_deg_sea, time_s_zeroed, thetaX_deg_complementary- 1.4)
xlabel('Time [s]')
ylabel('Angle [deg]')
legend('OxTS RT3100 estimation', 'ASM330LHH complementary')
ylim([-10 10])
Plotter(1)
