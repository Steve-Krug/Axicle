function [angleAccel_LP, angleComp, angleAccelRaw, anglePureIntegration_HP, gyro_LPHP_dps, gyro_HP_dps] = Axicle_Rate2Angle_Comp_v1_bandpass(time_s, rate_dps, Ay_g, Az_g, tau_s, tau_s_hp, tau_s_lp_gyro)
...
gyro_LPHP_dps = zeros(length(rate_dps), 1);
...
for i = 2:length(rate_dps)
    %rate_dps(i) = sbg_rate_dps(i); % Uncomment to use SBG

    gyro_LPHP_dps(i) = ( rate_dps(i) * alpha_hp) + (1-alpha_hp) * gyro_LPHP_dps(i-1); %Low pass the gyro data

    gyro_HP_dps(i) = (rate_dps(i) - gyro_LPHP_dps(i)) ; %High pass the gyro data from the low pass

    gyro_LP_dps(i) = ( rate_dps(i) * alpha_lp_gyro) + (1-alpha_lp_gyro) * gyro_LP_dps(i-1); %Low pass the gyro data
    ...
    if isnan(thetaX_Accel_null_deg)
        thetaX_Accel_deg(i) = 0;
    else
        thetaX_Accel_deg(i) = thetaX_Accel_null_deg;
    end
    ...
    thetaX_GyroIntegration_deg(i) = (thetaX_deg_Complementary_deg(i-1) + gyro_LP_dps(i)*dt); %Pure integration. thetaX_deg_comp(i-1) was the error from before (used to be thetaX_deg_fromIntegration(i))

    thetaX_Integration_HP_deg(i) = (thetaX_Integration_HP_deg(i-1) + gyro_LP_dps(i)*dt); %Pure integration. thetaX_deg_comp(i-1) was the error from before (used to be thetaX_deg_fromIntegration(i))
