...
% 1/22/24 - v15 changed to hindry 554p
clear all
close all
clc
...
time_sbg_zeroed_s = (sbgData.TimeStamp - sbgData.TimeStamp(1)) + (-1.432 + 4.792);
...
lowerClip_s = 0; %section with 2deg static dwell
upperClip_s = 1000;
...
% tau_s = linspace(0.05, 0.9, 50);
% errorRMSE = zeros(length(tau_s),1);

% for i = 1:length(tau_s)
% 
%     [~, ~, ~, thetaX_HP_deg] = Rate2Angle_Complementary_HP_v6(time_s_clipped, Wx_dps_clipped, Ay_g_clipped, Az_g_clipped, tau_s(i), tau_s_hp);
...
% minval_tau = min(tau_s);


% [rateDrift, thetaX_accel, thetaX_comp_deg, thetaX_LP_deg, thetaX_HP_deg] = Rate2Angle_Complementary_HP_v7(time_s_clipped, Wx_dps_clipped, Ay_g_clipped, Az_g_clipped, tau_s, tau_s_hp);

[thetaX_comp_deg, thetaX_accel] = Rate2Angle_Complementary_HP_v8(time_s_clipped, Wx_dps_clipped, Ay_g_clipped, Az_g_clipped, tau_s, tau_s_hp);
...
subplot(2,1,2)
plot(time_s_clipped, thetaX_accel, time_s_clipped, thetaX_comp_deg)
xlim([lowerClip_s upperClip_s])
ylim([-25 25])
legend('thetaX Accel', 'ASM Complementary')
Plotter(1)
