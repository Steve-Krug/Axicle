function [rateDriftStore, angleAccel, angleComp, angleLP, angleHP] = Rate2Angle_Complementary_HP_v7(time_s, rate_dps, Ay_g, Az_g, tau_s, tau_s_hp)
% Rate2Angle_v6.m
% 1/24/24
% S. Krug
% Notes: v7 for creating moving average of roll rate
...
thetaX_deg_LP(1) = 0;
static_offset_deg = 0;
thetaX_deg_loop1 = 0; %static angle offset at t= 0? only if zero wasn't calibrated in the SBG software 1.5095
...
for i = 2:length(rate_dps)
    ...
    if i > 5000
        ...
        rateDrift = (thetaX_deg_fromIntegration_noRateSubtraction(i-1)-...
            thetaX_deg_fromIntegration_noRateSubtraction(i-4900))/(time_s(i-1) - time_s(i-4900));
        rateDriftStore(j) = rateDrift;
        rateDriftScaler = 0;
        j = j+1;
    end
    
    thetaX_deg_loop1 = (thetaX_deg_loop1 + rate_dps(i)*dt); %Pure integration
    thetaX_deg_loop2 = (thetaX_deg_loop2 + (rate_dps(i)-rateDrift)*dt) ; %Pure integration + tau_s*rateDrift(i)

    thetaX_deg_fromIntegration_noRateSubtraction(i) = thetaX_deg_loop1;
    thetaX_deg_fromIntegration_RateSubtraction(i) = thetaX_deg_loop2 + rateDrift*rateDriftScaler;



    thetaX_deg_comp(i) = ( alpha * thetaX_deg_fromIntegration_RateSubtraction(i) ) + ( (1-alpha) * thetaX_deg_LP_xl(i) ); %Apply acceleration complementary filter
    %
    ...
    thetaX_deg_HP(i) = thetaX_deg_comp(i);
