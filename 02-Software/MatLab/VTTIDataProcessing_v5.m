...
files = dir('D:\Timeseries\*FD4_*.csv');
...
for i = 1:13 %13 'FD' data sets
    ...
    index_Ax = strcmp(fullpaths,'D:\Timeseries\FD4_Accel_X_' + string(i)+ '.csv');
    all_data_Ax = table2array(readtable(fullpaths(index_Ax)));
    fprintf('Ax Loaded...1/6  ')
    index_Ay = strcmp(fullpaths,'D:\Timeseries\FD4_Accel_Y_' + string(i)+ '.csv');
    all_data_Ay = table2array(readtable(fullpaths(index_Ay)));
    fprintf('Ay Loaded...2/6  ')
    index_Az = strcmp(fullpaths,'D:\Timeseries\FD4_Accel_Z_' + string(i)+ '.csv');
    all_data_Az = table2array(readtable(fullpaths(index_Az)));
    fprintf('Az Loaded...3/6  ')
    index_Wx = strcmp(fullpaths,'D:\Timeseries\FD4_Gyro_X_' + string(i)+ '.csv');
    all_data_Wx = table2array(readtable(fullpaths(index_Wx)));
    fprintf('Wx Loaded...4/6  ')
    index_Wy = strcmp(fullpaths,'D:\Timeseries\FD4_Gyro_Y_' + string(i)+ '.csv');
    all_data_Wy = table2array(readtable(fullpaths(index_Wy)));
    fprintf('Wy Loaded...5/6  ')
    index_Wz = strcmp(fullpaths,'D:\Timeseries\FD4_Gyro_Z_' + string(i)+ '.csv');
    ...
    for j = 1:length(new_id_Ax) %there are 1000 individual data sets per "FD"
        ...
        if length(Wx_dps) < 2000
            rollFlag1 = false;
        elseif abs(mean(Wx_dps(1:2000))) > 2.0 %sometimes the gyros are statically broken/reading high, ignore data set if this happens
            rollFlag1 = false;
        end
