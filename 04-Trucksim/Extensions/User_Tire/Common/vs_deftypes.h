...
#define VS_DEFTYPES_H_
  ...
  ***/
  ...
  typedef enum {
      ...
      VS_KEYWORD_TAB_2D = 112, VS_KEYWORD_TAB_GAIN = 113,
      VS_KEYWORD_TAB_OFFSET = 114, VS_KEYWORD_TAB_CONST = 115,
      ...
      VS_KEYWORD_DEF_FUNC_FUNC = 138, VS_KEYWORD_DEF_FUNC_ID = 139,
      VS_KEYWORD_LSYM = 140, VS_KEYWORD_TOKEN_FUNC = 141
  } vs_keyword_type;
  ...
  typedef enum {  // start with attributes for outputs & exports
    ...
    ANI_NAME = 52,
    // checking values for par or sypar: 0 don't check, -1 value is locked,
    // 1 check for min only, 2 check for max only, 3 check for both min and max
    PAR_CHECK_VALUE = 53, PAR_MIN_VALUE = 54, PAR_MAX_VALUE = 55,
    SYS_PAR_CHECK_VALUE = 56, SYS_PAR_MIN_VALUE = 57, SYS_PAR_MAX_VALUE = 58,
    ISYM_PTR_ARR = 59, PAR_VS_DEFINE = 60
  } vs_sym_attr_type;
  ...
  typedef int (*status_func_t)(vs_ext_loc where, int statusCode, vs_real statusValue,
     void* userData);

  /// Definition of optional external terrain function
