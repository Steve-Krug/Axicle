...
methods (Access=protected, Hidden)
    ...
    % Check the IPC file and update the waitbar with progress.
        ...
        if ishandle(this.wbh),
            if this.percent > .01 && this.percent ~= this.last_percent
                this.last_percent = this.percent;
                this.estimated_time_hours = ((toc(this.tot_toc)/this.percent)-toc(this.tot_toc))/(24*60*60);
                waitbar( this.percent, this.wbh, ['Estimated time to finish (HH:MM:SS): ' datestr(this.estimated_time_hours, 'HH:MM:SS')] );
            elseif this.percent > .01
                this.estimated_time_hours = this.estimated_time_hours - (.5/(24*60*60));
                if this.estimated_time_hours < 1/(24*60*60)
                    this.estimated_time_hours = 1/(24*60*60);
                end
                waitbar( this.percent, this.wbh, ['Estimated time to finish (HH:MM:SS): ' datestr(this.estimated_time_hours, 'HH:MM:SS')] );                  
            else
