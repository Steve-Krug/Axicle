function tsout = rdivide(ts1,ts2)
...
if isa(ts1,'timeseries')
    if numel(ts1)~=1
        error(message('MATLAB:timeseries:rdivide:noarray'));
    end
    if isa(ts2,'timeseries')
        if numel(ts2)~=1
           error(message('MATLAB:timeseries:rdivide:noarray'));
        end
        tsout = localrdivide(ts1,ts2);
    elseif isnumeric(ts2) || islogical(ts2)
        % Use option 'false' for ts\N
        tsout = localrdivide(ts1, ts2, false);
    else
    ...
    if isnumeric(ts1) || islogical(ts1)
        ...
        tsout = localrdivide(ts2,ts1,true);
    else
        error(message('MATLAB:timeseries:arith:typemix', class( ts1 )));
    end
end

function tsout = localrdivide(ts1, ts2, varargin)
    ...
    if ts1.IsTimeFirst == ts2.IsTimeFirst
        ...
        if ts1.DataInfo.isstorage
            try
                ...
                [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(ts1.DataInfo,ts2.DataInfo,ts1.Data,ts2.Data);
            end
        else
            [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(ts1.DataInfo,ts2.DataInfo,ts1.Data,ts2.Data);
        end
...
elseif isnumeric(ts2) || islogical(ts2)
    ...
    if isscalar(ts2)
        if isArrayFirst
            if ts1.DataInfo.isstorage
                try
                    tsout.DataInfo = rdivide(ts2,ts1.DataInfo);
                catch
                    [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide([],ts1.DataInfo,ts2,ts1.Data);   
                end
            else
                [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide([],ts1.DataInfo,ts2,ts1.Data);
            end
            ...
            if ts1.DataInfo.isstorage
                try
                   tsout.DataInfo = rdivide(ts1.DataInfo,ts2);
                catch
                   [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(ts1.DataInfo,[],ts1.Data,ts2);
                end
            else
                [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(ts1.DataInfo,[],ts1.Data,ts2);
            end
        ...
        if ~isArrayFirst
            if ts1.IsTimeFirst 
                if ts1.DataInfo.isstorage
                    try
                       tsout.DataInfo = rdivide(ts1.DataInfo,reshape(repmat(ts2,ts1.Length,1),size(ts1.Data)));
                    catch
                        [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(...
                ...
                if ts1.DataInfo.isstorage
                    try
                        tsout.DataInfo = rdivide(ts1.DataInfo,reshape(repmat(ts2,1,ts1.Length),size(ts1.Data)));
                    catch
                        [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(...
            ...
            if ts1.IsTimeFirst
                if ts1.DataInfo.isstorage
                    try
                        tsout.DataInfo = rdivide(reshape(repmat(ts2,ts1.Length,1),size(ts1.Data)),ts1.DataInfo);
                    catch
                        [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(...
                ...
                if ts1.DataInfo.isstorage
                    try
                        tsout.DataInfo = rdivide(reshape(repmat(ts2,1,ts1.Length),size(ts1.Data)),ts1.DataInfo);
                    catch
                        [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(...
